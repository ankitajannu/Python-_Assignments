...
def MarvellousClassifier(DataPath):
    ...
    ##################################################################
    # Step 8 : Scatter Plot of StudyHours vs PreviousScore
    ##################################################################

    print(border)
    print("Step 8 : Scatter Plot of StudyHours vs PreviousScore")
    print(border)
    ...
    plt.scatter(passed_students["StudyHours"],passed_students["PreviousScore"],color="green",label="Passed")
    plt.scatter(failed_students["StudyHours"],failed_students["PreviousScore"],color="blue",label="Failed")
    ...
    ##################################################################
    # Step 9 : BoxPlot of Attendance
    ##################################################################

    print(border)
    print("Step 9 : BoxPlot of Attendance")
    print(border)
    ...
    first_quarile = df["Attendance"].quantile(0.25)
    third_quartile = df["Attendance"].quantile(0.75)

    interquartile_range = third_quartile - first_quarile

    lower_limit = first_quarile - 1.5 * interquartile_range
    upper_limit = third_quartile + 1.5 * interquartile_range

    outliers = df[(df["Attendance"] < lower_limit) | (df["Attendance"] > upper_limit)]

    if len(outliers) > 0:
        print("Outliers are present in Attendance column")
        print(outliers[["Attendance"]])
    else:
        print("No outliers are present in Attendance column")

    ##################################################################
    # Step 10 : Plot AssignmentsCompleted vs FinalResult
    ##################################################################

    print(border)
    print("Step 10 : Plot AssignmentsCompleted vs FinalResult")
    print(border)
    ...
    print("Observations : ")
    print("1.The graph shows the relationship between AssignmentsCompleted vs FinalResult.")
    print("2.Students who completed more assignments are more likely to pass.")
    print("3.Passed students are more concentrated at higher assignments completion values.")
    print("4.Students with fewer completed assignments have a higher chance of failing.")
    print("5.This indicates that completed assignments positively affects academic performance.")

    ##################################################################
    # Step 11 : Plot SleepHours vs FinalResult
    ##################################################################

    print(border)
    print("Step 11 : Plot SleepHours vs FinalResult")
    print(border)
    ...
    print("Observations : ")
    print("1.The graph shows the relationship between SleepHours vs FinalResult.")
    print("2.Both passed and failed students are distributed across different sleep hour values.")
    print("3.Students with moderate sleep hours tend to perform better.")
    print("4.Sleeping more alone does not guarantee success.")
    print("5.Other factors such as StudyHours, Attendance, and AssignmentsCompleted also affect FinalResult.")
